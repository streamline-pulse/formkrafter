...
<Aside type="note">
`readOnly` signifie *cette valeur ne peut pas changer*, `disabled` signifie *ce
contrôle est indisponible* — et le rendu diffère. `readOnly` pose l'attribut
HTML `readonly` sur les champs texte et les zones de texte : la valeur reste
sélectionnable, copiable et atteignable au clavier, ce qu'exige un panneau de
consultation. Les contrôles pour lesquels HTML ne définit pas `readonly` —
select, case à cocher, radio, fichier — retombent sur `disabled` et portent
`aria-readonly`, pour que les technologies d'assistance entendent la nuance.
Les actions de collection (ajout, suppression, réordonnancement) restent
`disabled` dans les deux cas : ce sont des actions, pas des valeurs. La
soumission est neutralisée dans les deux cas.
</Aside>
